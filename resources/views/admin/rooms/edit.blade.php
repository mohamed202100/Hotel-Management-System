            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
                        <h3 class="text-3xl font-extrabold text-indigo-500">{{ __('Editing Room') }}
                            #{{ $room->room_number }}</h3>
                    ...
                    <!-- Form -->
                    <form method="POST" action="{{ route('admin.rooms.update', $room->id) }}" class="mt-6 space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="type" name="type"
                                ...
                                required>
                                <option value="Single" {{ old('type', $room->type) == 'Single' ? 'selected' : '' }}>
                                    {{ __('Single') }}</option>
                                <option value="Double" {{ old('type', $room->type) == 'Double' ? 'selected' : '' }}>
                                    {{ __('Double') }}</option>
                                <option value="Triple" {{ old('type', $room->type) == 'Triple' ? 'selected' : '' }}>
                                    {{ __('Triple') }}</option>
                                <option value="Family" {{ old('type', $room->type) == 'Family' ? 'selected' : '' }}>
                                    {{ __('Family') }}</option>
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="base_price" name="base_price" type="number" step="0.01" min="0"
                                class="mt-1 block w-full" :value="old('base_price', $room->base_price)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('base_price')" />
                        </div>

                        <!-- Status (Using string values) -->
                        <div>
                            ...
                            <select id="status" name="status"
                                ...
                                @php
                                    ...
                                    // Determine the current room status value. We now use $room->status which we assume is stored as a string.
...
<script>
    ...
    // Initial run to set capacity based on the default selected value (important for initial load)
    updateCapacity();
</script>
